...
sub transmute_to_lean_format {
    ...
    TESTCASE:   for ($_step_index = 0; $_step_index < $_numsteps; $_step_index++) {  ## no critic(ProhibitCStyleForLoops)
...
sub output_includes_less_than_testnum {
    ...
    my $_include_index;
    for ($_include_index = 0; $_include_index < $_numincludes; $_include_index++) {  ## no critic(ProhibitCStyleForLoops)
